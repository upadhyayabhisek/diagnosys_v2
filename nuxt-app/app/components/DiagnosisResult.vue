  ...
  >
    <div class="flex justify-between items-center mb-6">
      ...
      <div class="flex gap-3">
        <button
          @click="$emit('download')"
          class="bg-[var(--card)] border border-[var(--border)] p-3 rounded-xl hover:border-[var(--primary)] transition-all shadow-sm"
          title="Download PDF"
        >
          <i class="fas fa-file-pdf text-[var(--primary)]"></i>
        </button>
        <button
          class="bg-[var(--card)] border border-[var(--border)] p-3 rounded-xl hover:border-[var(--primary)] transition-all shadow-sm"
          title="Share Report"
          ...
          <div class="relative flex items-center justify-center">
            ...
            <svg class="w-24 h-24 transform -rotate-90">
              ...
              <circle
                ...
                :stroke-dashoffset="251.2 - (251.2 * Number(confidence)) / 100"
      ...
      <div class="p-10">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-4">
          ...
          >
            ...
            <span class="text-sm font-black tracking-tight text-[var(--text)]">
              {{ value || "N/A" }}
            </span>
          </div>
        </div>
      ...
      <div class="p-8 bg-[var(--background)] border-t border-[var(--border)]">
        <div class="flex items-start gap-4">
          ...
          <p class="text-[10px] font-medium leading-relaxed opacity-60">
            ...
            treatment. Please consult with a qualified healthcare provider to
            interpret these results in a clinical context.
          </p>
        </div>
      </div>
    </div>
    ...
    <div
      v-if="!patientName"
      class="mt-8 p-6 rounded-[2rem] bg-orange-500/5 border border-orange-500/20 text-center"
    >
      <p class="text-xs font-bold text-orange-600">
        <i class="fas fa-lock mr-2"></i> You are viewing this as a guest.
        <a href="#" class="underline font-black ml-1">Login</a> to save this
        report to your permanent health profile.
      </p>
    </div>
  </div>
</template>
...
<style scoped>
/* Scoped overrides if needed, but mostly uses your project's Tailwind/CSS variables */
@keyframes fade-in {
  from {
    opacity: 0;
  }
