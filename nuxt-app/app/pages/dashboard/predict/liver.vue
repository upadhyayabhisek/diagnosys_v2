...
const handleSubmit = async () => {
  ...
  try {
    const response: any = await $fetch(
      "http://localhost:5001/api/predict/liver",
      {
        method: "POST",
        body: payload,
      },
    ...
    alert(error.data?.message || "Failed connecting to Liver AI server.");
  } finally {
    ...
    <div v-else class="max-w-5xl">
      <header class="mb-12">
        ...
        <h1 class="text-5xl font-black tracking-tight mb-4">
          Liver Disease <br />
          <span class="opacity-50">Hepatic Analysis</span>
        </h1>
      </header>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        >
          ...
          <form @submit.prevent="handleSubmit" class="space-y-6">
            ...
            <button
              ...
              class="w-full mt-8 bg-[var(--primary)] text-white py-5 rounded-2xl font-black text-xs tracking-[0.2em] uppercase hover:brightness-110 active:scale-[0.98] transition-all shadow-lg shadow-[var(--primary)]/20 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <span v-if="isSubmitting">
                <i class="fas fa-circle-notch fa-spin mr-2"></i>
                Running Hepatic Analysis...
              </span>

              <span v-else> Run Hepatic Diagnostic → </span>
            </button>
          </form>
        </section>
        ...
        <aside class="space-y-6">
          ...
          >
            ...
            >
              This module evaluates 10 metabolic markers to detect cellular
