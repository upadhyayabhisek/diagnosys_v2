...
const handleSubmit = async () => {
  ...
  try {
    const response: any = await $fetch(
      "http://localhost:5001/api/predict/diabetes",
      {
        method: "POST",
        body: payload,
      },
    ...
    if (response.status === "success") {
      ...
      alert("Error calculating result: " + response.message);
    }
  } catch (error: any) {
    console.error("API Error:", error);
    const msg =
      error.data?.message || "Failed to connect to the diagnostic server.";
    alert(msg);
  } finally {
        ...
        >
          ...
          <form @submit.prevent="handleSubmit" class="space-y-6">
            ...
            >
              ...
              <span v-else>Run AI Diagnostic &rarr;</span>
            </button>
          </form>
        </section>
